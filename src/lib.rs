    ...
    T: Document<T> + Serialize + DeserializeOwned + Clone + Sync + Send + 'static + Debug,
{
    ...
    /// An In-Memory DB.
    pub fn new(fp: Option<PathBuf>, byte_length_increment: Option<usize>) -> Self {
        let mut file = None;
        if fp.is_some() {
            let path = fp.as_ref().unwrap();
            let f = fs::OpenOptions::new()
                ...
                .open(path);
            match f {
                Ok(_) => file = Some(f.unwrap()),
                Err(_) => {
                    println!("{} > Error creating/opening DB file", Utc::now());
                }
            }
        }
    ...
    pub fn new_arc(
        ...
        let c = Collection::new(fp, byte_length_increment);
...
mod test {
    ...
    fn test_insert() {
        ...
        let mut c = Collection::<User>::new(Some(fp), None);
